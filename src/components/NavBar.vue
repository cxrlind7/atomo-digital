  ...
  <header class="navbar-header">
    <nav class="navbar-container">
      ...
      <div class="nav-menu" :class="{ 'is-active': isMenuOpen }">
        ...
        <router-link to="/cotiza" class="cta-button" @click="closeMenu">
          Cotiza tu Proyecto
        </router-link>
      </div>
...
.nav-menu {
  ...
  align-items: center;
  gap: 2.5rem;
  /* Espacio entre la lista de enlaces y el botón CTA */
}
...
.logo {
  display: flex;
  /* ¡La magia está aquí! Activa Flexbox */
  align-items: center;
  /* Centra verticalmente el ícono y el texto */
  gap: 12px;
  /* Añade un espacio de 12px entre el ícono y el texto */
...
.logo-icon {
  ...
  background: white;
  /* El fondo base ahora es blanco */
  overflow: hidden;
  /* Oculta lo que se salga del círculo */
}
...
.logo-icon::after {
  ...
  position: absolute;
  width: 150%;
  /* Más grande para evitar bordes al rotar */
  height: 150%;
...
/* Página activa: el enlace actual queda resaltado */
...
.hamburger {
  ...
  padding: 0;
  z-index: 1001;
  /* Debe estar por encima del menú desplegable */
}
...
@media (max-width: 992px) {
  ...
  .nav-links a {
    font-size: 1.5rem;
    /* Hacemos la fuente más grande para que sea fácil de tocar */
    font-weight: 600;
  }
